...
gst_v4l2_open (GstV4l2Object * v4l2object)
{
  ...
  if (v4l2object->extended_control)
...
gboolean
gst_v4l2_set_extended_controls (GstV4l2Object * v4l2object, struct v4l2_ext_controls * ctrl)
{
  if (!GST_V4L2_IS_OPEN (v4l2object))
    return FALSE;

  GST_INFO_OBJECT(v4l2object, "About to set %d controls (%p)\n", ctrl->count, ctrl);
  if (v4l2_ioctl (v4l2object->video_fd, VIDIOC_S_EXT_CTRLS, ctrl) < 0)
  {
    if (ctrl->error_idx == ctrl->count) {
      GST_ELEMENT_WARNING (v4l2object->element, RESOURCE, SETTINGS,
                (_("Failed to validate the ctrl on device %s"), v4l2object->videodev), GST_ERROR_SYSTEM);
    } else if (ctrl->error_idx < ctrl->count) {
      GST_ELEMENT_WARNING (v4l2object->element, RESOURCE, SETTINGS,
                (_("Failed to set control %d on device %s\n"), ctrl->controls[ctrl->error_idx].id, v4l2object->videodev), GST_ERROR_SYSTEM);
    }
    return FALSE;
  }
  return TRUE;
