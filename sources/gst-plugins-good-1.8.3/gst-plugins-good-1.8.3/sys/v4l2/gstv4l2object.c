...
#include <gst/v4l2/gstv4l2meta.h>
#include <gst/parrot/gstparrotelementmeta.h>
    ...
    const char *default_device)
{
  ...
  /**
   * GstV4l2Src: extended-controls:
   *
   */
  g_object_class_install_property (gobject_class, PROP_EXTENDED_CONTROLS,
      g_param_spec_pointer ("extended-controls", "Extended Controls",
          "Extended v4l2 controls passed as v4l2_ext_controls pointer",
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  ...
  /**
   * GstV4l2Object:disable-copy-threshold:
   *
   * When set to true, element will never copy buffers if it's running low
   * on buffer.
   *
   * Since: 1.6
   */
  g_object_class_install_property (gobject_class, PROP_DISABLE_COPY_THRESHOLD,
      g_param_spec_boolean ("disable-copy-threshold", "Disable copy threshold",
          "Never copy buffers when we are running low on buffers",
          FALSE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

  g_object_class_install_property (gobject_class, PROP_MIN_BUFFERS,
      g_param_spec_uint ("min-buffers", "Minimum buffers",
          "Number of minimum buffers to allocate/request",
          GST_V4L2_MIN_BUFFERS, VIDEO_MAX_FRAME, GST_V4L2_MIN_BUFFERS,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
}
    ...
    GstV4l2UpdateFpsFunction update_fps_func)
{
  ...
  v4l2object->disable_copy_threshold = FALSE;
  v4l2object->user_min_buffers = GST_V4L2_MIN_BUFFERS;
    ...
    guint prop_id, const GValue * value, GParamSpec * pspec)
{
  switch (prop_id) {
    ...
    case PROP_EXTENDED_CONTROLS:{
      struct v4l2_ext_controls *p = g_value_get_pointer (value);

      v4l2object->extended_control = p;
      if (GST_V4L2_IS_OPEN (v4l2object))
        gst_v4l2_set_extended_controls (v4l2object, v4l2object->extended_control);
      break;
    }
    ...
    case PROP_FORCE_ASPECT_RATIO:
      ...
      break;
    case PROP_DISABLE_COPY_THRESHOLD:
      v4l2object->disable_copy_threshold = g_value_get_boolean (value);
      break;
    case PROP_MIN_BUFFERS:
      v4l2object->user_min_buffers = g_value_get_uint (value);
      break;
    default:
      return FALSE;
      break;
    ...
    guint prop_id, GValue * value, GParamSpec * pspec)
{
  switch (prop_id) {
    ...
    case PROP_FORCE_ASPECT_RATIO:
      ...
      break;
    case PROP_DISABLE_COPY_THRESHOLD:
      g_value_set_boolean (value, v4l2object->disable_copy_threshold);
      break;
    case PROP_MIN_BUFFERS:
      g_value_set_uint (value, v4l2object->user_min_buffers);
      break;
    case PROP_EXTENDED_CONTROLS:
      g_value_set_pointer (value, v4l2object->extended_control);
      break;
    default:
      return FALSE;
      break;
...
gst_v4l2_object_decide_allocation (GstV4l2Object * obj, GstQuery * query)
{
  ...
  has_v4l2_meta = gst_query_find_allocation_meta (query,
      GST_V4L2_META_API_TYPE, NULL);
  ...
  if (pushing_from_our_pool) {
    ...
    own_min = MAX (own_min, obj->user_min_buffers);
  ...
  if (has_v4l2_meta) {
    GST_DEBUG_OBJECT (obj->element, "activate V4l2 Meta");
    gst_buffer_pool_config_add_option (config,
        GST_BUFFER_POOL_OPTION_V4L2_META);
  }

  /* activate Parrot element meta unconditionally */
  gst_buffer_pool_config_add_option (config,
      GST_BUFFER_POOL_OPTION_PARROT_ELEMENT_META);
...
gst_v4l2_object_propose_allocation (GstV4l2Object * obj, GstQuery * query)
{
  ...
  min = MAX (min, obj->user_min_buffers);

  gst_query_add_allocation_pool (query, pool, size, min, max);

  /* we also support various metadata */
  gst_query_add_allocation_meta (query, GST_VIDEO_META_API_TYPE, NULL);
  gst_query_add_allocation_meta (query, GST_V4L2_META_API_TYPE, NULL);
  gst_query_add_allocation_meta (query, GST_PARROT_ELEMENT_META_API_TYPE, NULL);
