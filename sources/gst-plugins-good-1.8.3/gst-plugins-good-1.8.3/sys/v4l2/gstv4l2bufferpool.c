    ...
    GstBuffer * src)
{
  ...
  GstV4l2Meta *v4l2meta;
  GstParrotElementMeta *meta;
  ...
  meta = gst_buffer_get_parrot_element_meta (src);
  if (meta) {
    const GstMetaInfo *info = GST_META_CAST (meta)->info;
    ...
    info->transform_func (dest, GST_META_CAST (meta), src,
        _gst_meta_transform_copy, &copy);
  }

  GST_CAT_LOG_OBJECT (CAT_PERFORMANCE, pool, "slow copy into buffer %p", dest);

  v4l2meta = gst_buffer_get_v4l2_meta (src);
  if (v4l2meta) {
    const GstMetaInfo *info = GST_META_CAST (v4l2meta)->info;
    GstMetaTransformCopy copy;
    ...
    info->transform_func (dest, GST_META_CAST (v4l2meta), src,
        _gst_meta_transform_copy, &copy);
  }
    ...
    GstBufferPoolAcquireParams * params)
{
  ...
  if (pool->add_v4l2meta)
    gst_buffer_add_v4l2_meta (newbuf);

  if (pool->add_parrotelementmeta)
    gst_buffer_add_parrot_element_meta (newbuf);
...
gst_v4l2_buffer_pool_set_config (GstBufferPool * bpool, GstStructure * config)
{
  ...
  pool->add_v4l2meta = gst_buffer_pool_config_has_option (config,
      GST_BUFFER_POOL_OPTION_V4L2_META);
  pool->add_parrotelementmeta = gst_buffer_pool_config_has_option (config,
      ...
      switch (obj->mode) {
        ...
        case GST_V4L2_IO_DMABUF:
        {
          ...
          if ((*buf)->pool == bpool) {
            ...
            if (!obj->disable_copy_threshold &&
                num_queued < pool->copy_threshold) {
              GstBuffer *copy;
