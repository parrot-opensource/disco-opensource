...
gst_v4l2src_create (GstPushSrc * src, GstBuffer ** buf)
{
  ...
  /* save original v4l2 timestamp in meta */
  vmeta = gst_buffer_get_v4l2_meta (*buf);
  if (vmeta)
    vmeta->timestamp = timestamp;

  pmeta = gst_buffer_get_parrot_element_meta (*buf);
  if (pmeta)
    gst_structure_set (pmeta->structure, "capture-timestamp",
        GST_TYPE_CLOCK_TIME, timestamp, NULL);
