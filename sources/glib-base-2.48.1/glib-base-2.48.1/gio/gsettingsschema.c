...
/**
 * g_settings_schema_get_child_schema_id:
 * @schema: a #GSettingsSchema
 * @name: the name of the child schema
 *
 * Get the id of the child schema with @name in @schema.
 *
 * Returns: (nullable): the id of the child schema or %NULL if there is no child
 * with @name.
 *
 * Since: 2.50
 */
const gchar *
g_settings_schema_get_child_schema_id (GSettingsSchema *schema,
                                       const gchar *name)
{
  ...
  child_schema = g_settings_schema_get_string (schema, child_name);
  g_free (child_name);
