...
static void
on_srcpad_linked (GstPad * pad, GstPad * peer, gpointer user_data)
{
  GstTee *tee = GST_TEE (user_data);
  GstEvent *event;

  GST_DEBUG_OBJECT (tee, "pad linked");
  event = gst_event_new_reconfigure ();
  if (!gst_pad_push_event (tee->sinkpad, event)) {
    GST_WARNING_OBJECT (tee, "reconfigure event can't be sent");
  }
}
    ...
    const gchar * name_templ, const GstCaps * caps)
{
  ...
  g_signal_connect (srcpad, "linked", G_CALLBACK (on_srcpad_linked), tee);
...
static gboolean
gst_tee_handle_query_allocation (GstTee * tee, GstQuery * query)
{
  gboolean res;
  GList *pads;
  gboolean first_pad = TRUE;
  gboolean no_meta = FALSE;
  GstQuery *result;
  guint n, i;
  guint32 cookie;
  ...
  /* FIXME: Don't forget to unlock tee */
  GST_OBJECT_LOCK (tee);

  pads = GST_ELEMENT_CAST (tee)->srcpads;

  /* Special case for one pad */
  if (!pads->next) {
...
restart:
  ...
  result = gst_query_copy (query);

  while (pads) {
    GstPad *pad;
    GstQuery *tmp;

    /* Query each source pad and intersect their answer */
    pad = gst_object_ref (GST_PAD_CAST (pads->data));
    tmp = gst_query_copy (query);

    GST_OBJECT_UNLOCK (tee);
    res = gst_pad_peer_query (pad, tmp);
    GST_OBJECT_LOCK (tee);

    gst_object_unref (pad);

    if (G_UNLIKELY (GST_ELEMENT_CAST (tee)->pads_cookie != cookie)) {
      /* pads list has changed, re-query all remaining pads */
      GST_LOG_OBJECT (pad, "pad list changed");
      gst_query_unref (tmp);
      gst_query_unref (result);
      goto restart;
    }

    if (!res) {
      /* downstream element doesn't handle allocation query, assume it don't
       * bother about bufferpool and allocation, but strip all meta */
      gst_query_unref (tmp);
      pads = g_list_next (pads);
      GST_INFO_OBJECT (tee, "peer doesn't handle allocation query");
      no_meta = TRUE;
      continue;
    }

    /* Aggregate pools */
    n = gst_query_get_n_allocation_pools (tmp);
    for (i = 0; i < n; i++) {
      GstBufferPool *pool;
      guint size, min, max;

      gst_query_parse_nth_allocation_pool (tmp, i, &pool, &size, &min, &max);
      gst_query_add_allocation_pool (result, pool, size, min, max);

      if (pool)
        gst_object_unref (pool);
    }

    n = gst_query_get_n_allocation_params (tmp);
    for (i = 0; i < n; i++) {
      GstAllocator *allocator;
      GstAllocationParams param;

      gst_query_parse_nth_allocation_param (tmp, i, &allocator, &param);
      gst_query_add_allocation_param (result, allocator, &param);
    ...
    if (first_pad) {
      n = gst_query_get_n_allocation_metas (tmp);
      for (i = 0; i < n; i++) {
        GType api;
        const GstStructure *params;

        api = gst_query_parse_nth_allocation_meta (tmp, i, &params);
        gst_query_add_allocation_meta (result, api, params);
        first_pad = FALSE;
      }
    } else {
      /* intersect with first pad supported meta */
      GstQuery *tmp2;

      tmp2 = gst_query_intersect_allocation_meta (result, tmp);
      gst_query_replace (&result, tmp2);
      gst_query_unref (tmp2);
    }
  ...
  /* Copy result to initial query */
  n = gst_query_get_n_allocation_pools (result);
  for (i = 0; i < n; i++) {
    GstBufferPool *pool;
    guint size, min, max;

    gst_query_parse_nth_allocation_pool (result, i, &pool, &size, &min, &max);
  ...
  n = gst_query_get_n_allocation_params (result);
  for (i = 0; i < n; i++) {
    GstAllocator *allocator;
    GstAllocationParams param;

    gst_query_parse_nth_allocation_param (result, i, &allocator, &param);
    gst_query_add_allocation_param (query, allocator, &param);
  ...
  if (!no_meta) {
    /* Add allocation meta to allocation query answer */
    n = gst_query_get_n_allocation_metas (result);
    for (i = 0; i < n; i++) {
      GType api;
      const GstStructure *params;

      api = gst_query_parse_nth_allocation_meta (result, i, &params);
...
gst_tee_sink_query (GstPad * pad, GstObject * parent, GstQuery * query)
{
  ...
  switch (GST_QUERY_TYPE (query)) {
    case GST_QUERY_ALLOCATION:
      res = gst_tee_handle_query_allocation (GST_TEE (parent), query);
      break;
    default:
