...
/**
 * gst_query_intersect_allocation_meta:
 * @query1: a GST_QUERY_ALLOCATION type query #GstQuery to intersect
 * @query2: a GST_QUERY_ALLOCATION type query #GstQuery to intersect
 *
 * Creates a new GstQuery from @query1 that contains all the metas that are
 * common to both @query1 and @query2.
 *
 * Returns: the new #GstQuery
 */
GstQuery *
gst_query_intersect_allocation_meta (GstQuery * query1, GstQuery * query2)
{
  GstQuery *res;
  GstStructure *s;
  const GValue *value;
  guint n_allocation_metas;
  GType api;
  guint i;

  res = gst_query_copy (query1);

  /* gst_query_copy won't deep copy result GArray as it is a refcounted object
   * so just remove result array for meta from res query and add
   * common meta. */
  s = GST_QUERY_STRUCTURE (res);
  value = gst_structure_id_get_value (s, GST_QUARK (META));
  if (value) {
    gst_structure_remove_field (s, "meta");
  }

  n_allocation_metas = gst_query_get_n_allocation_metas (query1);
  for (i = 0; i < n_allocation_metas; i++) {
    /* FIXME: what to do with meta params ? */
    api = gst_query_parse_nth_allocation_meta (query1, i, NULL);
    if (gst_query_find_allocation_meta (query2, api, NULL))
      gst_query_add_allocation_meta (res, api, NULL);
  }
